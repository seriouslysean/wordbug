...
4. **`const` by default**: Flag `let` where `const` would work. `let` is only valid for genuine reassignment (loop counters, stream callbacks, stateful accumulation). Never `var`.
5. **Interaction with changed code**: Verify that callers and callees of changed functions still work correctly with the new signatures or behavior
6. **Security**: Exposed secrets, input validation gaps at system boundaries
